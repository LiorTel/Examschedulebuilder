...
def _event_name_from_source(source_text: str) -> str:
    cleaned = source_text
    cleaned = RANGE_HEBREW_PATTERN.sub("", cleaned)
    cleaned = RANGE_MONTH_NAME_PATTERN.sub("", cleaned)
    cleaned = RANGE_SHORT_PATTERN.sub("", cleaned)
    cleaned = RANGE_FULL_PATTERN.sub("", cleaned)
    cleaned = SINGLE_MONTH_NAME_PATTERN.sub("", cleaned)
    cleaned = SINGLE_NUMERIC_PATTERN.sub("", cleaned)
    cleaned = re.sub(r"\s{2,}", " ", cleaned)
    cleaned = cleaned.strip(" -–:;,.()")
    return cleaned or source_text.strip()


def _append_candidate(candidates: list[dict], source_text: str, semester_context: Optional[str], in_no_exam_section: bool, start: date, end: Optional[date]) -> None:
    start_time, end_time = extract_times(source_text)
    candidates.append(
        {
            "event_name": _event_name_from_source(source_text),
...
def extract_date_candidates(blocks: list[dict], academic_year: str) -> list[dict]:
    ...
    for block in blocks:
        text = normalize_hebrew_text(block.get("text", ""))
        if not text:
            continue
        ...
        for pattern in [RANGE_HEBREW_PATTERN, RANGE_FULL_PATTERN]:
            for match in pattern.finditer(text):
                ...
                if start:
                    _append_candidate(candidates, text, semester_context, in_no_exam_section, start, end)
        ...
        for match in SINGLE_MONTH_NAME_PATTERN.finditer(text):
            ...
            if parsed:
                _append_candidate(candidates, text, semester_context, in_no_exam_section, parsed, None)
                consumed.append(match.span())
